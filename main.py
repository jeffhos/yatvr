...
FILE_REGEXES = [
    re.compile(
        r"(?P<show_name>[^/]+?)\s*\((?P<year>\d{4})\)/Season (?P<season>\d+)/", re.I
    ),
    ...
    re.compile(r"Part\s*(?P<episode>\d+)", re.I),
]
...
def find_matching_shows(guessed_show_name: str, year: Optional[str]) -> list[ShowInfo]:
    ...
    if len(shows) == 1:
        ...
        raise NoMatchingShowException("[ERROR] No show chosen")
...
def rename_episode(
    file: Path, show_info: ShowInfo, season: Optional[int], episode: int
):
    if season is None:
...
def rename_video(
    file: Path, show_name: str, year: Optional[str], season: int, episode: int
):
